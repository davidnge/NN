...
public class FourNeighbour {
    ...
    {
        fourMatrix = new double [(lattice.getSOMHEIGHT())][(lattice.getSOMWIDTH())];
    ...
    {
        for (int i=0; i<latticeInput.getSOMHEIGHT(); i++)
        {
            for (int j=0; j<latticeInput.getSOMWIDTH(); j++)
            {     
                ...
                else if(i>0 && j>0 && i<(fourMatrix.length-1) && j<(fourMatrix[0].length-1))
